...
bool ExportNVM(const Reconstruction& reconstruction,
  ...
  for (const auto image_id : reconstruction.RegImageIds()) {
    ...
    if (skip_distortion ||
      ...
      LOG(MM_WARNING) << "NVM only supports `SIMPLE_RADIAL` "
...
bool ExportCam(const Reconstruction& reconstruction,
  ...
  for (const auto image_id : reconstruction.RegImageIds()) {
    ...
    if (skip_distortion ||
      ...
      LOG(MM_WARNING) << "CAM only supports `SIMPLE_RADIAL`, `RADIAL`, "
                      "and pinhole camera models.\n";
      return false;
    }
...
bool ExportRecon3D(const Reconstruction& reconstruction,
  ...
  for (const auto image_id : reconstruction.RegImageIds()) {
    ...
    if (skip_distortion ||
      ...
      LOG(MM_WARNING) << "Recon3D only supports `SIMPLE_RADIAL`, "
...
bool ExportBundler(const Reconstruction& reconstruction,
  ...
  for (const image_t image_id : reconstruction.RegImageIds()) {
    ...
    if (skip_distortion ||
      ...
      LOG(MM_WARNING) << "Bundler only supports `SIMPLE_RADIAL`, "
                      "`RADIAL`, and pinhole camera models.\n";
      return false;
    }
