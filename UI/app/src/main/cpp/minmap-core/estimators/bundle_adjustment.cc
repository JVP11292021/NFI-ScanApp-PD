...
// camera pairs within a rig, etc.
        void FixGaugeWithTwoCamsFromWorld(
            ...
            if (image1 == nullptr || image2 == nullptr) {
                LOG(MM_WARNING) << "Failed to fix Gauge with two cameras. "
            ...
            DefaultBundleAdjuster(BundleAdjustmentOptions options,
                ...
                switch (config_.FixedGauge()) {
                    ...
                    case BundleAdjustmentGauge::THREE_POINTS:
                        ...
                        break;
                    default:
                        LOG(MM_FATAL) << "Unknown BundleAdjustmentGauge";
                }
            }
        ...
        public:
            ...
            void AddPosePriorToProblem(image_t image_id,
                ...
                if (!prior.IsValid() || !prior.IsCovarianceValid()) {
                    LOG(MM_ERROR) << "Could not add prior for image #" << image_id;
                    return;
                }
            ...
            bool AlignReconstruction() {
                ...
                if (prior_options_.ransac_max_error > 0) {
                    ...
                    if (num_valid_covs == 0) {
                        LOG(MM_WARNING) << "No pose priors with valid covariance found.";
                        return false;
                    }
                ...
                LOG(MM_INFO) << "Robustly aligning reconstruction with max_error="
                ...
                if (success) {
                    ...
                    LOG(MM_WARNING) << "Alignment w.r.t. prior positions failed";
                }
    ...
    void PrintSolverSummary(const ceres::Solver::Summary& summary,
        ...
        LOG(MM_INFO) << summary.FullReport();
