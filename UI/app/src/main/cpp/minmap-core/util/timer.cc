...
void Timer::PrintSeconds() const {
  LOG(MM_INFO) << StringPrintf("Elapsed time: %.5f [seconds]", ElapsedSeconds());
}

void Timer::PrintMinutes() const {
  LOG(MM_INFO) << StringPrintf("Elapsed time: %.3f [minutes]", ElapsedMinutes());
}

void Timer::PrintHours() const {
  LOG(MM_INFO) << StringPrintf("Elapsed time: %.3f [hours]", ElapsedHours());
}
