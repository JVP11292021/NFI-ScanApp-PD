...
void PrintElapsedTime(const Timer& timer) {
  LOG(MM_INFO) << StringPrintf("in %.3fs", timer.ElapsedSeconds());
}
  ...
  void Run() override {
    ...
    while (std::getline(file, line)) {
      ...
      try {
        ...
        LOG(MM_ERROR) << "Could not read image pair.";
        break;
      }
      ...
      if (image_name_to_image.count(image_name1) == 0) {
        LOG(MM_INFO) << StringPrintf("SKIP: Image %s not found in database.",
                                  image_name1.c_str());
        break;
      }
      ...
      if (database_->ExistsInlierMatches(image1.ImageId(), image2.ImageId())) {
        LOG(MM_INFO) << "SKIP: Matches for image pair already exist in database.";
        skip_pair = true;
      }
      ...
      while (std::getline(file, line)) {
        ...
        try {
          ...
          LOG(MM_ERROR) << "Cannot read feature matches.";
          break;
        }
