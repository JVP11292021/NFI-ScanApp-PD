...
void IterativeGlobalRefinement(const IncrementalPipelineOptions& options,
  ...
  LOG(MM_INFO) << "Retriangulation and Global bundle adjustment";
...
void ExtractColors(const std::string& image_path,
  ...
  if (!reconstruction.ExtractColorsForImage(image_id, image_path)) {
    LOG(MM_WARNING) << StringPrintf("Could not read image %s at path %s.",
...
void WriteSnapshot(const Reconstruction& reconstruction,
  ...
  LOG(MM_WARNING) << "=> Writing to " << path;
...
void IncrementalPipeline::Run() {
  ...
  for (size_t i = 0; i < kNumInitRelaxations; ++i) {
    ...
    LOG(MM_INFO) << "=> Relaxing the initialization constraints.";
...
bool IncrementalPipeline::LoadDatabase() {
  ...
  if (database_cache_->NumImages() == 0) {
    LOG(MM_WARNING) << "No images with matches found in the database";
    return false;
  }
...
IncrementalPipeline::Status IncrementalPipeline::InitializeReconstruction(
  ...
  if (!options_->IsInitialPairProvided()) {
    LOG(MM_INFO) << "Finding good initial image pair";
    ...
    if (!find_init_success) {
      LOG(MM_INFO) << "=> No good initial image pair found.";
    ...
    if (!provided_init_success) {
      LOG(MM_INFO) << "=> Provided pair is unsuitable for initialization.";
  ...
  LOG(MM_INFO) << "Global bundle adjustment";
  ...
  do {
    ...
    for (size_t reg_trial = 0; reg_trial < next_images.size(); ++reg_trial) {
      ...
      if (reg_next_success) {
        ...
        LOG(MM_INFO) << "=> Could not register, trying another image.";
  ...
  for (int num_trials = 0; num_trials < options_->init_num_trials;
    ...
    switch (status) {
      case Status::INTERRUPTED: {
        LOG(MM_INFO) << "Keeping reconstruction due to interrupt";
      ...
      case Status::NO_INITIAL_PAIR: {
        LOG(MM_INFO) << "Discarding reconstruction due to no initial pair";
      ...
      case Status::BAD_INITIAL_PAIR: {
        LOG(MM_INFO) << "Discarding reconstruction due to bad initial pair";
        mapper.EndReconstruction(/*discard=*/true);
        reconstruction_manager_->Delete(reconstruction_idx);
      ...
      case Status::SUCCESS: {
        ...
        if ((options_->multiple_models && reconstruction_manager_->Size() > 1 &&
          ...
          LOG(MM_INFO) << "Discarding reconstruction due to insufficient size";
          mapper.EndReconstruction(/*discard=*/true);
          reconstruction_manager_->Delete(reconstruction_idx);
        } else {
          LOG(MM_INFO) << "Keeping successful reconstruction";
          mapper.EndReconstruction(/*discard=*/false);
          AlignReconstructionToOrigRigScales(database_cache_->Rigs(),
                                             reconstruction.get());
        }
      ...
      default:
        LOG(MM_FATAL) << "Unknown reconstruction status.";
    }
  }
}
...
void IncrementalPipeline::TriangulateReconstruction(
  ...
  LOG(MM_INFO) << "Iterative triangulation";
  size_t image_idx = 0;
  for (const image_t image_id : reconstruction->RegImageIds()) {
    const auto& image = reconstruction->Image(image_id);
