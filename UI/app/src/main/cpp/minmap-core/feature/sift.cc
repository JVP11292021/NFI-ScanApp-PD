...
void WarnDarknessAdaptivityNotAvailable() {
  LOG(MM_WARNING) << "Darkness adaptivity only available for GLSL SiftGPU.";
}
        ...
        if (vl_keypoints[i].is != prev_level) {
          if (i > 0) {
            ...
            if (descriptors != nullptr) {
              level_descriptors.back().conservativeResize(static_cast<int>(level_idx), 128);
            }
          }
        ...
        for (int o = 0; o < num_used_orientations; ++o) {
          ...
          if (descriptors != nullptr) {
            ...
            if (options_.normalization ==
              ...
              LOG(MM_FATAL) << "Normalization type not supported";
            }
...
std::unique_ptr<FeatureExtractor> CreateSiftFeatureExtractor(
  ...
  if (options.estimate_affine_shape || options.domain_size_pooling ||
    ...
    LOG(MM_INFO) << "Creating Covariant SIFT CPU feature extractor";
    return CovariantSiftCPUFeatureExtractor::Create(options);
  } else {
    LOG(MM_INFO) << "Creating SIFT CPU feature extractor";
    ...
    for (FeatureDescriptors::Index i2 = 0; i2 < descriptors2.rows(); ++i2) {
      if (guided_filter != nullptr && guided_filter((*keypoints1)[i1].x,
        ...
        if (distance_type == DistanceType::L2) {
          ...
          LOG(MM_FATAL) << "Distance type not supported";
        }
      } else {
        if (distance_type == DistanceType::L2) {
          distances(i1, i2) =
...
std::unique_ptr<FeatureMatcher> CreateSiftFeatureMatcher(const SiftMatchingOptions& options) {
    LOG(MM_INFO) << "Creating SIFT CPU feature matcher";
