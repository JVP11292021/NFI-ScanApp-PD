...
static colmap::retrieval::VisualIndex::Descriptors LoadRandomDatabaseDescriptors(
    ...
    colmap::retrieval::VisualIndex::Descriptors descriptors;
    ...
    if (max_num_images < 0) {
        ...
        colmap::RandomSampler random_sampler(max_num_images);
    ...
    for (const size_t image_idx : image_idxs) {
        const auto& image = images.at(image_idx);
        const colmap::FeatureDescriptors image_descriptors =
...
static std::vector<colmap::Image> ReadVocabTreeRetrievalImageList(
    ...
    if (path.empty()) {
        ...
        const std::vector<std::string> image_names = colmap::ReadTextFileLines(path);
        images.reserve(image_names.size());
        for (const auto& image_name : image_names) {
            colmap::Image image = database->ReadImageWithName(image_name).value();
...
int RunVocabTreeBuilder(int argc, char** argv) {
    std::string vocab_tree_path = colmap::kDefaultVocabTreeUri;
    colmap::retrieval::VisualIndex::BuildOptions build_options;
    ...
    auto visual_index = colmap::retrieval::VisualIndex::Create();
...
int RunVocabTreeRetriever(int argc, char** argv) {
    ...
    auto visual_index = colmap::retrieval::VisualIndex::Read(vocab_tree_path);
    ...
    for (size_t i = 0; i < database_images.size(); ++i) {
        ...
        if (max_num_features > 0 && descriptors.rows() > max_num_features) {
            colmap::ExtractTopScaleFeatures(&keypoints, &descriptors, max_num_features);
        }
        ...
        LOG(INFO) << colmap::StringPrintf(" in %.3fs", timer.ElapsedSeconds());
    }
    ...
    for (size_t i = 0; i < query_images.size(); ++i) {
        ...
        LOG(INFO) << colmap::StringPrintf("Querying for image %s [%d/%d]",
        ...
        for (const auto& image_score : image_scores) {
            ...
            LOG(INFO) << colmap::StringPrintf("  image_id=%d, image_name=%s, score=%f",
