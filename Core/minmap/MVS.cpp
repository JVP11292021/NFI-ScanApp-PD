...
int RunDelaunayMesher(int argc, char** argv) {
    ...
    colmap::StringToLower(&input_type);
    if (input_type == "sparse") {
        colmap::mvs::SparseDelaunayMeshing(
            *options.delaunay_meshing, input_path, output_path);
    }
    else if (input_type == "dense") {
        colmap::mvs::DenseDelaunayMeshing(
            *options.delaunay_meshing, input_path, output_path);
    }
    else {
...
int RunPatchMatchStereo(int argc, char** argv) {
    ...
    colmap::StringToLower(&workspace_format);
    ...
    colmap::mvs::PatchMatchController controller(*options.patch_match_stereo,
        workspace_path,
        workspace_format,
        pmvs_option_name,
        config_path);
...
int RunPoissonMesher(int argc, char** argv) {
    ...
    THROW_CHECK(
        colmap::mvs::PoissonMeshing(*options.poisson_meshing, input_path, output_path));
...
int RunStereoFuser(int argc, char** argv) {
    ...
    colmap::mvs::StereoFusion fuser(*options.stereo_fusion,
        workspace_path,
        workspace_format,
        pmvs_option_name,
        input_type);
    ...
    if (workspace_format == "colmap") {
        reconstruction.Read(colmap::JoinPaths(workspace_path, "sparse"));
    }
    ...
    if (output_type == "bin") {
        ...
        colmap::mvs::WritePointsVisibility(output_path + ".vis",
